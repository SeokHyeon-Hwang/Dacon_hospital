rm(list=ls())
trdf<-read.csv('E:/Bigdata/dataset/2ndCompetitionData/train.csv')
tedf<-read.csv('E:/Bigdata/dataset/2ndCompetitionData/test.csv')
'''
...
te_a$instkind = factor(te_a$instkind, levels = colN)

levels(tr_a$sido)
levels(te_a$sido)
levels(tr_a$instkind)
levels(te_a$instkind)
...
?randomForest
set.seed(30)
rf_model <- randomForest(OC~profit1, data=tr_sub_noNA)
rf_model
